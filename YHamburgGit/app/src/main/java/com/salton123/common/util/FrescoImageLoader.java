...
//        p_SimpleDraweeView.setHierarchy(_builder.build());
    ...
    private static ImagePipelineConfig configureCaches(Context context) {
        ...
        final MemoryCacheParams bitmapCacheParams = new MemoryCacheParams(
                ...
                me.nereo.multi_image_selector.utils.FrescoImageLoader.MAX_MEMORY_CACHE_SIZE, // 内存缓存中准备清除但尚未被删除的总图片的最大大小,以字节为单位。
...
//    		  .setDiskTrimmableRegistry(diskTrimmableRegistry)//类将包含一个注册表的缓存减少磁盘空间的环境。
                .setMaxCacheSize(me.nereo.multi_image_selector.utils.FrescoImageLoader.MAX_DISK_CACHE_SIZE)//默认缓存的最大大小。
