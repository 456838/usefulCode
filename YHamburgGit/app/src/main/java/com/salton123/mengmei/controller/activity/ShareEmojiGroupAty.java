 ...
 * Time: 11:54
 */
public class ShareEmojiGroupAty extends ActivityFrameIOS {

    private ExpandGridView gv_pictures;
    ...
    public void InitView() {
        gv_pictures = (ExpandGridView) findViewById(R.id.gv_pictures);
    ...
    private void ShowTipDialog() {
        AlertDialog.Builder _builder = new AlertDialog.Builder(ShareEmojiGroupAty.this);
    ...
    private void UploadImageModel() {
        ...
        BmobDataEngine.UploadMultiImage(ListToArray(mSelectPath), new UploadBatchListener() {
            ...
            public void onSuccess(List<BmobFile> list, List<String> list1) {
                if (list1.size() == mSelectPath.size()) {//如果数量相等，则代表文件全部上传完成
                    System.out.println("size" + list.size());
                    LogUtils.e("size" + list.size());
                    if (mUser != null) {
                        EmojiGroup _EmojiGroup = new EmojiGroup();
                        _EmojiGroup.setKeyWord("");
                        _EmojiGroup.setOwner(mUser);
                        BmobRelation _realtion = new BmobRelation();
                        _realtion.add(User.getCurrentUser());
                        _EmojiGroup.setLinks(_realtion);
                        StringBuffer sb = new StringBuffer();
                        for (int i = 0; i < list.size(); i++) {
                            String fileUrl = list.get(i).getUrl();
                            sb.append(fileUrl).append(";");
                            ImageBean imageBean = new ImageBean();
                            imageBean.setKeyWord("emoji");
                            imageBean.setOwner(mUser);
                            imageBean.setLinks(_realtion);
                            imageBean.setPicUrl(fileUrl);
                            imageBean.setProvider(mUser.getNickname());
                        ...
                        _EmojiGroup.setPicUrl(sb.toString());
                        _EmojiGroup.save(new SaveListener<String>() {
                            @Override
                            public void done(String s, BmobException e) {
                                if (e == null) {
            ...
            if (position == getCount() - 1) {
                ...
                iv_thumbnail.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        Intent intent = new Intent(ShareEmojiGroupAty.this, MultiImageSelectorActivity.class);
                        // 是否显示拍摄图片
                ...
                FrescoImageLoader.displayLocalThumbnailImage(iv_thumbnail, url, 100, 100);
                iv_thumbnail.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
